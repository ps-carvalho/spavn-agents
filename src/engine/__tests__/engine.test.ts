...
describe("Seed", () => {
  ...
  it("seedDatabase returns correct counts", () => {
    ...
    expect(result.targets).toBe(1);   // opencode only
  });
...
describe("SpavnEngine", () => {
  ...
  it("renderAgent delegates to the OpenCode renderer", () => {
    engine = new SpavnEngine(dbPath);
    engine.initialize();
    const output = engine.renderAgent("architect", "opencode");
    expect(output).toContain("description:");
